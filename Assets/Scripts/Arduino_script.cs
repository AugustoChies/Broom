    ...
    void Update()
    {
        ...
        if (!dataString.Equals("NOT OPEN"))
        {
            ...
            if (start)
            {
                if (calibration)
                {
                    calibrot = Quaternion.Euler(curr_angle_x * factor, -curr_angle_y * factor, curr_angle_z * factor);
                    calibration = false;
                }
                else
                {
                    Quaternion rotaual = Quaternion.Euler(curr_angle_x * factor, -curr_angle_y * factor, curr_angle_z * factor);

                    if (enableTranslation) target.transform.position = new Vector3(curr_offset_x, curr_offset_z, curr_offset_y);
                    if (enableRotation) target.transform.localRotation = Quaternion.Euler(rotaual.eulerAngles - calibrot.eulerAngles);
                 }
            }
        }
    }
